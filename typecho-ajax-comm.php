...
var
loading_dom = '<div id="ajax_comment_loading"><?php echo $config->loadingLan; ?><\/div><div id="ajax_comment_error"><\/div>',
...
jQuery(document).ready(function() {
	...
	comments       = jQuery(_comments),
	comment_form   = jQuery(_comment_form),
	respond        = jQuery(_respond),
	textarea       = jQuery(_comment_form +' '+ _textarea),
	$submit        = jQuery(_comment_form +' '+ _submit),
	parent = '';
	textarea.parent().before(loading_dom);
	jQuery('#ajax_comment_loading').hide();
	err = jQuery('#ajax_comment_error').hide();
	...
	comment_form.submit(function() {
		...
		jQuery('#ajax_comment_loading').slideDown();
        ...
        jQuery.ajax({
			...
			success: function(data) {
				jQuery('#ajax_comment_loading').slideUp();

				try {
					if (data.indexOf(_comment_list) == -1){ // return error
						var msg = data.match(/<\/h1>([\s\S]+?)<\/div>/);
                        err.html($.trim(msg[1]));
						err_effect(); 
						return false;
					} else {
						...
						parent != '' ? ( jQuery('#' + parent + ' li').length && comments_order == 'DESC' ? jQuery('#' + parent + ' li:first').before(data) : respond.before(data),
								parent = '') : (respond.before('<ol '+ _comment_list + '>' + data + '<\/ol>'));
						
						jQuery('#comment-' + new_id).hide().fadeIn(1000); 
						comments.length ? ( n = parseInt(comments.text().match(/\d+/)), comments.text(comments.text().replace( n, n + 1 ))) : 0;
			
						$body.animate({ scrollTop: jQuery('#comment-' + new_id).offset().top - 200 }, 900);
						jQuery('textarea').each(function() {this.value = ''});
						setTimeout(function() {jQuery('#ajax_comment_success').slideUp();}, 2000);
					}
				} catch (e) {
					alert('Error!\n\n' + e );
				}
				...
				jQuery(_comment_reply + ' a, #cancel-comment-reply-link').unbind('click'); click_bind();//new comment bind
				jQuery('#author').length ? countdown() : $submit.attr('disabled', false).fadeTo('slow', 1);
			}
		});
	...
	function click_bind() { // bind
		jQuery(_comment_reply + ' a').click(function() { // replay
			$body.animate({ scrollTop: respond.offset().top - 180 }, 400);
			h = jQuery(this)[0].href;
			parent = 'comment-' + h.substring(h.indexOf('replyTo=') + 8, h.indexOf('#'));
			textarea.focus();
		});
		jQuery('#cancel-comment-reply-link').click(function() { // cancle
			parent = '';
		});	
	}
